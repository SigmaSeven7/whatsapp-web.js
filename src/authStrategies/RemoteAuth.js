...
//         this.backupSyncIntervalMs = backupSyncIntervalMs;
//         this.dataPath = path.resolve(dataPath || './.wwebjs_auth/');
//         this.requiredDirs = ['Default', 'IndexedDB', 'Local Storage']; // Required directories
//     }
...
//         const sessionExists = await this.store.sessionExists({ session: this.sessionName });
//         if (!sessionExists) {
//             await this.delay(60000); // Initial delay for session stability
//             await this.storeRemoteSession({ emit: true });
//         }
...
//         const pathExists = await this.isValidPath(this.userDataDir);
//         if (pathExists) {
//             const buffer = await this.compressSession();
//             await this.store.save({ session: this.sessionName, data: buffer });
//             if (options && options.emit) this.client.emit(Events.REMOTE_SESSION_SAVED);
//         }
//     }
...
//         }
//         if (sessionExists) {
//             const buffer = await this.store.extract({ session: this.sessionName });
//             await this.unCompressSession(buffer);
//         } else {
//             fs.mkdirSync(this.userDataDir, { recursive: true });
//         }
//     }
...
//     async unCompressSession(buffer) {
 ...
 * @param {object} options - options
 * @param {object} options.store - MongoDB store instance
 * @param {string} options.clientId - Client id to distinguish instances
 * @param {number} options.backupSyncIntervalMs - Backup interval (min 60000ms)
 */
class RemoteAuth extends BaseAuthStrategy {
    constructor({ clientId, store, backupSyncIntervalMs } = {}) {
        ...
        if (!store) throw new Error('MongoDB store is required.');

        this.store = store;
        this.clientId = clientId;
        this.backupSyncIntervalMs = backupSyncIntervalMs;
        this.sessionName = this.clientId ? `RemoteAuth-${this.clientId}` : 'RemoteAuth';
        this.requiredDirs = ['Default', 'IndexedDB', 'Local Storage'];
    }

    async beforeBrowserInitialized() {
        const puppeteerOpts = this.client.options.puppeteer;
        const dirPath = await this.store.createTempUserDataDir(this.sessionName);

        if (puppeteerOpts.userDataDir && puppeteerOpts.userDataDir !== dirPath) {
            throw new Error('MongoRemoteAuth is not compatible with a user-supplied userDataDir.');
        }
    ...
    async disconnect() {
        await this.deleteRemoteSession();
        await this.store.cleanupTempDir(this.userDataDir);
        clearInterval(this.backupSync);
    }
    ...
    async storeRemoteSession(options) {
        if (await this.store.isValidPath(this.userDataDir)) {
            const sessionData = await this.store.compressSessionData(this.userDataDir, this.requiredDirs);
            await this.store.saveSession(this.sessionName, sessionData);
            if (options && options.emit) this.client.emit(Events.REMOTE_SESSION_SAVED);
        }
    }
    ...
    async extractRemoteSession() {
        const sessionExists = await this.store.sessionExists(this.sessionName);
        if (sessionExists) {
            const sessionData = await this.store.getSession(this.sessionName);
            await this.store.extractSessionData(sessionData, this.userDataDir);
        }
    }

    async deleteRemoteSession() {
        const sessionExists = await this.store.sessionExists(this.sessionName);
        if (sessionExists) await this.store.deleteSession(this.sessionName);
    }
